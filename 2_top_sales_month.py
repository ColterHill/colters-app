...
def fetch_closed_won_opportunities(sf: Salesforce, account_ids: List[str]) -> pd.DataFrame:
    ...
    if not df.empty:
        ...
        df["MonthName"] = df["CloseDate"].dt.strftime("%B")
    return df


def compute_top_sales_months(opps_df: pd.DataFrame, accounts_df: pd.DataFrame) -> pd.DataFrame:
    if opps_df.empty or accounts_df.empty:
        return pd.DataFrame(columns=[
            "AccountId", "AccountName", "Rank", "Month", "AverageMonthlyAmount", "TotalTransactions", "YearsWithData"
        ])

    account_id_to_name = dict(zip(accounts_df["Id"], accounts_df["Name"]))
    
    # Group by Account, Month and compute metrics across all years
    monthly_stats = opps_df.groupby(["AccountId", "Month", "MonthName"], as_index=False).agg(
        TotalAmount=("Amount", "sum"),
        TransactionCount=("Id", "count"),
        YearsWithData=("Year", "nunique"),
    )
    
    # Calculate average monthly amount (total amount / years with data)
    monthly_stats["AverageMonthlyAmount"] = monthly_stats.apply(
        lambda r: (r["TotalAmount"] / r["YearsWithData"]) if r["YearsWithData"] else 0.0,
        axis=1,
    )
    
    # Find top 3 months for each account based on average monthly amount
    top_months = []
    for account_id in monthly_stats["AccountId"].unique():
        account_data = monthly_stats[monthly_stats["AccountId"] == account_id]
        if not account_data.empty:
            # Sort by average monthly amount descending and take top 3
            top_3 = account_data.nlargest(3, "AverageMonthlyAmount")
            ...
            for rank, (_, row) in enumerate(top_3.iterrows(), 1):
                top_months.append({
                    ...
                    "Rank": rank,
                    "Month": row["MonthName"],
                    "AverageMonthlyAmount": row["AverageMonthlyAmount"],
                    "TotalTransactions": row["TransactionCount"],
                    "YearsWithData": row["YearsWithData"],
                })
    
    result_df = pd.DataFrame(top_months)
...
def main():
    ...
    print("Computing top 3 sales months...")
    result_df = compute_top_sales_months(opps_df, accounts_df)

    output_dir = "/Users/colterhill/Documents/Marketing/Salesforce Top Contractor data"
    os.makedirs(output_dir, exist_ok=True)
    output_path = os.path.join(output_dir, "top_sales_month.csv")
