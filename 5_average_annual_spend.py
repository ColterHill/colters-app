...
def compute_average_annual_spend(opps_df: pd.DataFrame, accounts_df: pd.DataFrame) -> pd.DataFrame:
    if opps_df.empty or accounts_df.empty:
        return pd.DataFrame(columns=[
            "AccountId", "AccountName", "YearsActive", "TotalAmount", "AverageAnnualSpend"
        ])

    account_id_to_name = dict(zip(accounts_df["Id"], accounts_df["Name"]))
    
    # Group by Account and Year, then by Account
    yearly_totals = opps_df.groupby(["AccountId", "Year"], as_index=False).agg(
        YearlyAmount=("Amount", "sum"),
    )
    
    # Now group by Account to get totals and year counts
    account_summary = yearly_totals.groupby("AccountId", as_index=False).agg(
        YearsActive=("Year", "count"),
        TotalAmount=("YearlyAmount", "sum"),
    )
    
    account_summary["AverageAnnualSpend"] = account_summary.apply(
        lambda r: (r["TotalAmount"] / r["YearsActive"]) if r["YearsActive"] else 0.0,
        axis=1,
    )
    account_summary["AccountName"] = account_summary["AccountId"].map(account_id_to_name)
    
    cols = ["AccountId", "AccountName", "YearsActive", "TotalAmount", "AverageAnnualSpend"]
    return account_summary[cols].sort_values("AccountName").reset_index(drop=True)
...
def main():
    ...
    print("Computing average annual spend...")
    result_df = compute_average_annual_spend(opps_df, accounts_df)

    output_dir = "/Users/colterhill/Documents/Marketing/Salesforce Top Contractor data"
    os.makedirs(output_dir, exist_ok=True)
    output_path = os.path.join(output_dir, "average_annual_spend.csv")
