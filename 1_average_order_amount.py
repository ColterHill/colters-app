...
def fetch_closed_won_opportunities(sf: Salesforce, account_ids: List[str]) -> pd.DataFrame:
    if not account_ids:
        return pd.DataFrame(columns=["Id", "AccountId", "Amount"])
...
def compute_average_order_amount(opps_df: pd.DataFrame, accounts_df: pd.DataFrame) -> pd.DataFrame:
    if opps_df.empty or accounts_df.empty:
        return pd.DataFrame(columns=["AccountId", "AccountName", "AverageOrderAmount", "TotalOrders"])

    account_id_to_name = dict(zip(accounts_df["Id"], accounts_df["Name"]))
    
    grouped = opps_df.groupby("AccountId", as_index=False).agg(
        TotalAmount=("Amount", "sum"),
        TotalOrders=("Id", "count"),
    )
    
    grouped["AverageOrderAmount"] = grouped.apply(
        lambda r: (r["TotalAmount"] / r["TotalOrders"]) if r["TotalOrders"] else 0.0,
        axis=1,
    )
    grouped["AccountName"] = grouped["AccountId"].map(account_id_to_name)
    
    cols = ["AccountId", "AccountName", "AverageOrderAmount", "TotalOrders"]
...
def main():
    ...
    print("Computing average order amounts...")
    result_df = compute_average_order_amount(opps_df, accounts_df)

    output_dir = "/Users/colterhill/Documents/Marketing/Salesforce Top Contractor data"
    os.makedirs(output_dir, exist_ok=True)
    output_path = os.path.join(output_dir, "average_order_amount.csv")
